...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Rain':
						ClientPrefs.rain = !ClientPrefs.rain;

					case 'Fog':
						ClientPrefs.fog = !ClientPrefs.fog;
					...
					case 'Note Splashes':
						ClientPrefs.noteSpaches = !ClientPrefs.noteSpaches;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Shaders':
				daText = "Shader that can slow down performance.";
			case 'Rain':
				daText = "Toggle background rain effect which can reduce performance.";
			case 'Fog':
				daText = "Toggle background fog effect which can reduce performance.";
			case 'Low Quality':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Rain':
						daValue = ClientPrefs.rain;
					case 'Fog':
						daValue = ClientPrefs.fog;
					case 'Shaders':
					...
					case 'Note Splashes':
						daValue = ClientPrefs.noteSpaches;
					case 'Flashing Lights':
